...
void sendSBUSFrame(uint8_t failsafe, uint8_t lostpack)
{
  ...
  if ((now - sOutLast) > 10000) {
    ...
    SerialWrite(Serial, SBUS_SYNC);
    for (uint8_t i = 0; i<23; i++) {
      SerialWrite(Serial, sbus.bytes[i]);
    }
    SerialWrite(Serial, SBUS_TAIL);
  }
}
...
void sendSUMDFrame(uint8_t failsafe)
{
  ...
  if ((now - sOutLast) > 10000) {
    ...
    SerialWrite(Serial, CRC16_value >> 8);
    SerialWrite(Serial, CRC16_value & 0xff);
  }
}
